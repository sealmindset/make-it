...
18a. **Re-verify on every code change.** After ANY change to code, configuration, or
     dependencies -- including fixes made during build-verify, a fix cycle, or debugging --
     re-run the checks that cover the changed area (tests, lint/type-check, build, and any
     affected live checks) before reporting status. A result from before the change is stale
...
20. **Linter and formatter configured** -- Python projects have Ruff configured in `pyproject.toml`. TypeScript/JavaScript projects have ESLint (flat config) and Prettier. Config files are generated by the scaffold, not left to the developer. `ruff check` and `npx eslint .` must produce zero errors on generated scaffold code.
21. **Pre-commit hooks configured** -- `.pre-commit-config.yaml` exists with hooks for linting, formatting, and secret detection (gitleaks). Hooks run automatically on commit. If `pre-commit` is not installed on the user's machine, note in TODO.md as a setup step.
