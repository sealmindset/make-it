...
**BN03** [AI+Brain] [BLOCK] **Brain context injection in BaseAgent** -- `BaseAgent._load_brain_context(user_id)` method exists. When `BRAIN_FEATURES_ENABLED=true`, `invoke()` and `stream()` call `_load_brain_context()` and include the result in the assembled prompt between system prompt and domain context. When `BRAIN_FEATURES_ENABLED=false`, `_load_brain_context()` returns empty string (no-op). Verify: set `BRAIN_FEATURES_ENABLED=true`, create a test memory for a user, send a chat message as that user -- confirm the memory content appears in the assembled prompt (log or debug output). Set `BRAIN_FEATURES_ENABLED=false` -- confirm memory is NOT included.
...
**BN05** [AI+Brain] [BLOCK] **User memory isolation** -- `GET /api/brain/memories` always scopes to `owner_id = current_user.id` for user-type memories. Users can see org and decision memories (shared by design). Users CANNOT see other users' personal memories. Verify: create user memories for user A, login as user B, GET /api/brain/memories -- confirm user B cannot see user A's memories. Confirm user B CAN see org-type memories.
