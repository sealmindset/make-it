...
| preflight 6 (production) | Look for production indicators by name only: a file such as `.env.production`, or key names that point at production (for example `grep -oE '^[A-Za-z_]*PROD[A-Za-z_]*=' .env*`, which prints names only). Never print or quote a value. Skip the `echo` lines: they show the sandbox's settings, not theirs. Do not ask the "continue? passive scans only" question. Nothing live is scanned, so just note any production indicators in the report. |
