...
Also see ~/.claude/make-it/references/not-copied-fixture.md for background.
EOF
...
assert_line "$OUT" 'claude-code:make-it/references/not-copied-fixture.md'
...
if ! MAKE_IT_DESKTOP_SKILLS_DIR="$TMPSKILLS/skills" bash "$BUILD" --check; then
  echo "FAIL: --check should pass -- claude-code:make-it/references/not-copied-fixture.md is not a real references/ link" >&2
  exit 1
fi
